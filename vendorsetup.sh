...
chmod a+x device/samsung/a14x/prebuilt/avb/mkbootimg
add_lunch_combo twrp_a14x-eng
...
# Target Device
export FOX_TARGET_DEVICES="a14x"
export TARGET_DEVICE_ALT="a14x"
export FOX_BUILD_DEVICE="a14x"
	
export_build_vars(){
	echo -e "${GREEN}Exporting build vars from the a14x tree${RESET}"
	...
	export FOX_USE_SPECIFIC_MAGISK_ZIP="$PWD/device/samsung/a14x/prebuilt/magisk/magdelta.zip"
...
set_env_var(){
        ...
        echo " export CURR_DEVICE=a14x"
        exit 1
}

var_not_eq(){
        echo -e "${RED_BACK}CURR_DEVICE not equal to a14x${RESET}"
        ...
        echo -e "${RED}Skipping a14x specific build vars...${RESET}"
}
